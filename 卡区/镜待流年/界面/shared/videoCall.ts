...
export function createCall(key: string, owner: string, contact: string, time: string) {
  const call: VideoCall = { id: id(), owner, contact, time, startedAt: Date.now(), status: 'calling', screen: '', memory: '', lines: [] };
...
export function videoCallOptions(call: VideoCall) {
  ...
  return {
    ...
    ordered_prompts: [
      { role: 'system', content: `这是${call.owner}与${call.contact}一对一的文字视频通话。根据世界书中的联系人身份、双方关系与既有通话内容扮演${call.contact}。画面用自然分段的文字呈现对方在镜头中的动作、表情、环境与口头回应。双方只能看到镜头呈现和听到通话传来的信息。玩家已输入的言行是承接依据，下一步由玩家决定。\n输出 ===VIDEOCALL=== 数据块：screen:本轮对方画面与回应；memory:从接通至今的自包含累计摘要，在旧摘要基础上保留重要早期事实和未完成事项。以 ===CALLEND=== 结束。` },
