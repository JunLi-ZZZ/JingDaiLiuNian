...
function replyTo(event: { kind: string; name: string }) {
  if (!['audience', 'gift', 'join', 'fan'].includes(event.kind) || room.value?.status !== 'live') return;
  inputMode.value = 'chat'; drafts.chat = `回复@${event.name} `; editInput();
}
function start() {
  try {
    refresh();
