<template>
  <section v-if="room" class="vc" :class="{ 'vc-connected': !!room.screen }">
    <header class="vc-header"><button title="收起通话" aria-label="收起通话" @click="$emit('close')"><CallIcon name="back" /></button><div><b>{{ room.contact }}</b><small>{{ room.status === 'ended' ? '通话记录' : room.pending ? '对方正在回应' : room.status === 'calling' ? '等待接通' : '视频通话' }}</small></div><span>{{ elapsed }}</span><button v-if="room.screen" title="分享通话记录" aria-label="分享通话记录" @click="$emit('share', room)"><CallIcon name="share" /></button></header>
    <div v-if="!room.screen" class="vc-waiting"><div class="vc-avatar">{{ room.contact.slice(0, 1) }}</div><b>{{ room.contact }}</b></div>
    <nav v-if="room.screen" class="vc-tabs" aria-label="通话视图"><button :class="{on: view === 'screen'}" @click="view = 'screen'">当前画面</button><button :class="{on: view === 'history'}" @click="view = 'history'">通话记录</button></nav>
    <div v-if="room.screen" v-show="view === 'screen'" ref="screenEl" class="vc-screen"><p>{{ room.screen }}</p></div>
    <div v-show="view === 'history'" ref="logEl" class="vc-log" aria-live="polite"><div v-for="line in historyLines" :key="line.id" class="vc-line" :class="{ 'vc-mine': line.name === room.owner }"><small>{{ line.name }}</small><p>{{ line.text }}</p><button v-if="line.status === 'failed' && room.status !== 'ended'" @click="retry">重试</button></div></div>
    <div v-if="room.pending" class="vc-loading" role="status" aria-label="正在生成"><i /><i /><i /></div>
    ...
    <footer v-if="room.status !== 'ended'" class="vc-controls"><button :disabled="!!room.pending || !!room.error" @click="input"><i><CallIcon name="chat" /></i><span>说点什么</span></button><button :disabled="!!room.pending || !!room.error" @click="run('continue', '')"><i><CallIcon name="play" /></i><span>继续</span></button><button class="vc-hang" @click="hang"><i><CallIcon name="hang" /></i><span>挂断</span></button></footer>
...
const view = ref('screen'); const screenEl = ref<HTMLElement>();
const historyLines = computed(() => room.value?.lines || []);
...
watch(() => room.value?.screen, () => nextTick(() => { if (screenEl.value) screenEl.value.scrollTop = 0; }));
...
<style scoped>
.vc .vc-screen{flex:1;min-height:0}.vc-tabs{display:flex;flex-shrink:0;background:#e5ece9;border-bottom:1px solid #ccd8d2}.vc-tabs button{flex:1;padding:10px;color:#71847b}.vc-tabs button.on{color:#204c38;box-shadow:inset 0 -2px #408064}
